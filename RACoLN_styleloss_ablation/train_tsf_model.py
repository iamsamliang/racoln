...
wandb.init(project="RACoLN", name=f'no_style_loss', entity="iamsamliang")
...
for epoch in range(config.max_epoch):
    for i, batch in enumerate(train_iter):
        ...
        if global_step%config.log_iter==0:
            logger.info(f"[Iteration {global_step}] SelfLoss : {round(self_loss.item(), 3)} CycleLoss : {round(cycle_loss.item(), 3)} StyleLoss : {loss_r} ContentLoss : {round(mseLoss.item(), 3)}")
