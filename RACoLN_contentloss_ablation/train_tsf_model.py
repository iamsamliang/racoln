...
wandb.init(project="RACoLN", name=f'no_contentLoss', entity="iamsamliang")
...
for epoch in range(config.max_epoch):
    for i, batch in enumerate(train_iter):
        ...
        wandb.log({"self_loss": self_loss, "loss_r": loss_r, "mseLoss": mseLoss, "cycle_loss": cycle_loss})
        ...
        if global_step%config.log_iter==0:
            logger.info(f"[Iteration {global_step}] SelfLoss : {round(self_loss.item(), 3)} CycleLoss : {round(cycle_loss.item(), 3)} StyleLoss : {round(loss_r.item(), 3)} ContentLoss : {mseLoss}")
        if global_step%config.eval_iter==0:
            ...
            wandb.log({"dev_loss": loss, "dev_accuracy": accuracy})
            
            # If works great on Dev, then save model
            if loss<minLoss:
                minLoss = loss
