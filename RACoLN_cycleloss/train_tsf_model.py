...
wandb.init(project="RACoLN", name=f'no_cycle_loss', entity="iamsamliang")
...
for epoch in range(config.max_epoch):
    for i, batch in enumerate(train_iter):
        ...
        if global_step%config.log_iter==0:
            logger.info(f"[Iteration {global_step}] SelfLoss : {round(self_loss.item(), 3)} CycleLoss : {cycle_loss} StyleLoss : {round(loss_r.item(), 3)} ContentLoss : {round(mseLoss.item(), 3)}")
